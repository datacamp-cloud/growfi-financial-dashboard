...
export async function GET() {
  ...
  const [accounts, recentTransactions, goals, monthlyStats] = await Promise.all([
    ...
    prisma.transaction.findMany({
      where: { userId }, orderBy: { date: "desc" }, take: 6,
      include: {
    ...
    prisma.transaction.groupBy({ by: ["type"], where: { userId, date: { gte: startOfMonth } }, _sum: { amount: true } }),
  ])
  ...
  const remainingRate = monthlyIncome > 0 ? Math.round((remainingAmount / monthlyIncome) * 100) : 0

  return NextResponse.json({
    user: { name: session.user.name, email: session.user.email },
    totalBalance, monthlyIncome, monthlyExpenses, remainingAmount, remainingRate,
    accounts, recentTransactions, goals,
  })
}
