...
export function CompoundCalculator() {
  ...
  // Pre-remplir avec les vraies donnees financieres de l'utilisateur
  useEffect(() => {
    Promise.all([
      fetch('/api/dashboard').then((r) => r.json()),
      fetch('/api/user/profile').then((r) => r.json()),
    ]).then(([dashboard, profile]) => {
      // Capital initial = solde total actuel
      if (dashboard?.totalBalance > 0) {
        setInitial(Math.round(dashboard.totalBalance))
      }
      // Contribution mensuelle = epargne mensuelle (revenus - depenses)
  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,360px)_1fr]">
      ...
      <Card className="h-fit backdrop-blur-xl">
        ...
        <CardContent className="flex flex-col gap-5">
          <MoneyField
            label={
              <TermTooltip
                ...
                definition="La somme d'argent que tu places au depart. Pre-rempli avec ton solde actuel."
              />
            }
            value={initial}
            onChange={setInitial}
          ...
          <MoneyField
            label={
              <TermTooltip
                ...
                definition="Le montant que tu ajoutes chaque mois. Pre-rempli avec ton epargne mensuelle (revenus - depenses)."
              />
            }
            value={monthly}
            onChange={setMonthly}
          ...
          <RateField
            label={
              <TermTooltip
                term="Taux d'Interet Annuel"
                definition="Le pourcentage que ton placement rapporte chaque annee. 8% est un taux raisonnable pour un investissement diversifie."
              />
            }
            value={rate}
            onChange={setRate}
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-muted-foreground">
              <TermTooltip
                term="Frequence de Capitalisation"
                definition="La frequence a laquelle les interets sont ajoutes a ton capital. Plus c'est frequent, plus ton argent croit vite."
              />
            </span>
      ...
      <div className="flex flex-col gap-4">
        ...
        <Card className="backdrop-blur-xl">
          <CardHeader>
            <CardTitle>Detail annee par annee</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-border hover:bg-transparent">
                    ...
                    <TableHead className="text-right">Montant verse</TableHead>
                    <TableHead className="text-right">Interets</TableHead>
                ...
                <TableBody>
                  {result.rows.map((row) => (
                    <TableRow key={row.year} className="border-border/60">
                      <TableCell className="text-sm">Annee {row.year}</TableCell>
