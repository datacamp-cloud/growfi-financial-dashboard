...
export function ProfilePage() {
  ...
  // Donnees affichees = profile (API) en priorite, session en fallback
      ...
      <Card className="backdrop-blur-xl">
        <CardContent className="flex flex-col items-center gap-4 text-center sm:flex-row sm:text-left">
          ...
          <div className="flex-1">
            ...
            {(displayProf || displayActivity) && (
              <p className="text-sm text-muted-foreground">
                {[displayProf, displayActivity].filter(Boolean).join(' · ')}
              </p>
            )}
            {displayAge && (
              <p className="text-xs text-muted-foreground">{displayAge} ans</p>
            )}
            {displayGoal && (
              <p className="mt-1 text-xs text-neon/80 italic">"{displayGoal}"</p>
            )}
          </div>
        ...
        <Card className="backdrop-blur-xl">
          <CardContent>
            <p className="text-xs text-muted-foreground">
              <TermTooltip
                term="Serie d'epargne"
                definition="Le nombre de mois consecutifs ou tu as atteint ton objectif d'epargne. Une longue serie est signe d'une excellente discipline financiere !"
              />
            </p>
