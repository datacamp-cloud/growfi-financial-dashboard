...
export function AddGoalModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/goals', {
      ...
      body: JSON.stringify({ name, icon, targetAmount: parseFloat(targetAmount), color, deadline: deadline || null }),
    })
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-wrap gap-2">
              {goalIcons.map((i) => (
                <button key={i.name} type="button" onClick={() => setIcon(i.name)} title={i.label} className={`flex size-10 items-center justify-center rounded-xl transition-colors ${icon === i.name ? 'bg-primary/20 ring-2 ring-primary' : 'bg-white/5 hover:bg-white/10'}`}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2">
              {colors.map((c) => (
                <button key={c} type="button" onClick={() => setColor(c)} className={`size-8 rounded-full transition-transform ${color === c ? 'scale-125 ring-2 ring-white' : ''}`} style={{ backgroundColor: c }} />
              ))}
            </div>
          </div>
