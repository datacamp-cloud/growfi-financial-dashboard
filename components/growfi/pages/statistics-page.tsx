...
export function StatisticsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Statistiques"
        subtitle="Comprends où va ton argent et comment tes finances évoluent."
        action={
          <ToggleGroup
            type="single"
            value={[period] as string[]}
            onValueChange={(value) => {
              if (value?.length) setPeriod(value[0] as Period)
            }}
          ...
          >
            {(Object.keys(periodLabels) as Period[]).map((item) => (
              <ToggleGroupItem
                key={item}
                value={item}
                className="rounded-lg px-3 text-xs data-[state=on]:bg-neon data-[state=on]:text-[#0a1a0c]"
              >
                {periodLabels[item]}
              </ToggleGroupItem>
            ))}
        ...
        <Card className="backdrop-blur-xl">
          <CardContent className="pt-6">
            <p className="text-xs text-muted-foreground">
              <TermTooltip
                term="Reste après dépenses"
                definition="Ce qu'il reste après avoir soustrait tes dépenses de tes revenus sur la période sélectionnée. Ce montant peut ensuite être épargné, investi ou rester disponible."
              />
            </p>
            <Money value={netAmount} className="mt-1 block text-lg font-extrabold" colored />
            <p className="mt-1 text-xs text-muted-foreground">{formatPercent(netRate, 0)} des revenus</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card className="backdrop-blur-xl">
          <CardHeader><CardTitle>Dépenses par période</CardTitle></CardHeader>
          <CardContent>
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={statsData} margin={{ top: 10, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tickLine={false} axisLine={false} fontSize={12} width={44} tickFormatter={(value) => formatFCFACompact(value as number)} />
        ...
        <Card className="backdrop-blur-xl">
          <CardHeader><CardTitle>Revenus vs dépenses</CardTitle></CardHeader>
          <CardContent>
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={statsData} margin={{ top: 10, right: 8, left: 0, bottom: 0 }}>
      ...
      <Card className="backdrop-blur-xl">
        ...
        <CardContent className="flex flex-col gap-4">
          {categoryStats.length === 0 ? (
            <p className="text-sm text-muted-foreground">Aucune dépense enregistrée ce mois-ci.</p>
          ) : (
              ...
              <div key={category.name} className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-muted-foreground">
                    <Money value={category.spent} suffix={false} className="text-foreground" /> · {formatPercent(category.share, 0)}
                  </span>
                </div>
                <StatBar percent={category.share} color={category.color} />
                <span className="text-xs text-muted-foreground">{formatPercent(category.share, 0)} de tes dépenses</span>
              </div>
            ))
          )}
        </CardContent>
